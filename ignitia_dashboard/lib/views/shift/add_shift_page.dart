...
class AddShiftPage extends StatefulWidget {
  const AddShiftPage({Key? key}) : super(key: key);
...
class _AddShiftPageState extends State<AddShiftPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: CustomAppBar(
          title: Strings.titleAddShiftPage,
        ),
        body: Stack(
          children: [
            Padding(
                          ...
                          CustomButton(
                            onTap: () async {
                              ...
                              var shiftModel = ShiftModel(
                                  ...
                                  startTime: _timeToString(selectedStartTime),
                                  endTime: _timeToString(selectedEndTime),
                                  description: descriptionController.text,
                                  statusId: selectedStatus?.id ?? 1);
                              var isSuccess =
                                  await viewModel.addShift(shiftModel);
                              if (isSuccess) {
