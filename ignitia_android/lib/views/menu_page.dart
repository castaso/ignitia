...
class MenuPage extends StatelessWidget {
        ...
        child: Container(
          ...
          child: ListView.builder(
              ...
              itemBuilder: (BuildContext context, int index) {
                ...
                if (item.hasChildren) {
                  return _parentItem(context, item, mediaSize.width > webWidth);
                }
                return ListTile(
                  contentPadding: const EdgeInsets.only(left: 0, right: 0),
                  title: _menuItem(item),
                  onTap: () {
                    _menuItemTapped(context, item.id, item.page, mediaSize.width > webWidth);
                  },
                );
              }),
        ),
  ...
  Widget _parentItem(BuildContext context, MenuItemModel parent, bool isWeb) {
    return Container(
      ...
      child: ExpansionTile(
        ...
        children: parent.children!.map((child) {
          return ListTile(
            ...
            onTap: () => _menuItemTapped(context, child.id, child.page, isWeb),
          );
        }).toList(),
      ),
    );
